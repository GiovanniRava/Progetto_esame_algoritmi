    ...
    while (!codaVuota(&cp)) {
        ...
        for (i = 0; i < 4; i++) {
            ...
            if (vX >= 0 && vX < n && vY >= 0 && vY < m) {
                long differenzaAltezza = altezze[uX][uY] - altezze[vX][vY];
                long nuovoCosto = costo[uX][uY] + costoCella + costoCella + costoAltezza * differenzaAltezza * differenzaAltezza;
...
void dijkstra(int **altezze, int n, int m, int costoCella, int costoAltezza, Cella **precedente, long **costo) {
    ...
    while (!codaVuota(&cp)) {
        ...
        for (i = 0; i < 4; i++) {
            ...
            if (vX >= 0 && vX < n && vY >= 0 && vY < m) {
                long differenzaAltezza = altezze[uX][uY] - altezze[vX][vY];
                long nuovoCosto = costo[uX][uY] + costoCella + costoAltezza * differenzaAltezza * differenzaAltezza;
                /*Aggiorno il percorso se ne trovo uno meno costoso*/
                if (nuovoCosto < costo[vX][vY]) {
                    costo[vX][vY] = nuovoCosto;
                    precedente[vX][vY].x = uX;
                    ...
                    nuovoNodo.cella.x = vX;
                    nuovoNodo.cella.y = vY;
                    nuovoNodo.costo = nuovoCosto + costoCella; /* aggiunge il costo della cella di destinazione*/
                    inserisciNodo(&cp, nuovoNodo);
                }
            }
        }
...
void dijkstra(int **altezze, int n, int m, int costoCella, int costoAltezza, Cella **precedente, long **costo) {
    ...
    while (!codaVuota(&cp)) {
        ...
        uX = nodoMin.cella.x;
        uY = nodoMin.cella.y;
        /*Calcolo le coordinate del nodo adiacente*/
        for (i = 0; i < 4; i++) {
            vX = uX + direzioneX[i];
            vY = uY + direzioneY[i];
            /*controllo che le coordinate siano valide, ovvero stiano dentro i limiti della matrice*/
